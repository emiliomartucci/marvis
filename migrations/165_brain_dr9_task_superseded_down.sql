-- Down-migration 165 — revert the DR9 / task_superseded / task_probably_done CHECK extensions.
--
-- Emergency rollback only. Rebuilds both tables with the ORIGINAL (narrower) CHECKs and
-- DROPS rows whose value the reverted CHECK would reject (rule_id='DR9',
-- signal_type='task_superseded', finding_type='task_probably_done'). Those rows are P4-F1
-- output; discarding them on a downgrade is the accepted cost (documented in the up-migration).

PRAGMA foreign_keys = OFF;

BEGIN IMMEDIATE;

-- ============================================================ A. brain_drift_signals (revert)
...
INSERT INTO brain_drift_signals_old
    SELECT * FROM brain_drift_signals
    WHERE rule_id <> 'DR9' AND signal_type <> 'task_superseded';
...
INSERT INTO brain_findings_old
    SELECT * FROM brain_findings
    WHERE finding_type <> 'task_probably_done';
...
DELETE FROM schema_versions WHERE version = 165;
