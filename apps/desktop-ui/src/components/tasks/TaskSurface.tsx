  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <span className="flex min-w-0 items-center gap-1.5 text-[10.5px] text-pir-text-muted">
          <span aria-hidden className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: color }} />
          <span className="truncate">{project?.name ?? task.project}</span>
        </span>
        <span className="shrink-0 font-mono text-[10px] text-pir-text-muted">{task.id.slice(-6)}</span>
      </div>
      <div className="mt-1.5 text-label font-medium leading-snug text-pir-text-primary">{task.title}</div>
