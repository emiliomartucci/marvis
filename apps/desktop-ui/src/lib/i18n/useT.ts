...
export function useT(): {
  ...
  // Two-pass locale: first client render MUST match the prerendered HTML
  // ("en", since the static export has no window). Reading
  // localStorage/navigator.language in the useState initializer made the
  // hydration render diverge on any non-English browser (every label IT vs EN
  // → React #418 → full client re-render that drops externally-applied <html>
  // classes like theme-v2). The real locale is applied right after mount.
