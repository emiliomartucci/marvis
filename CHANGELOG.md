...
### Added
- Engine groundwork toward 0.3.8 — all behind default-off flags, so a default install behaves exactly like v0.3.7. Opt-in pieces: prose chunking before embedding, a structural Knowledge-Graph lane in search ranking, span-grounded citations in the session brief, deterministic community detection over the graph, and bitemporal columns on `learnings` (`valid_from` / `invalid_at`; an additive migration that backfills existing rows and keeps them live).

_Pre-released for testing as `0.3.8bN`. Install with `pip install "marvisx-cli==0.3.8b1"` (or `uv tool install "marvisx-cli==0.3.8b1"`)._
