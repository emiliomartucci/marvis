...
_Pre-released for testing as `0.3.8bN`. Install with `pip install "marvisx-cli==0.3.8b6"` (or `uv tool install "marvisx-cli==0.3.8b6"`)._

_Note: `0.3.8b2` was **yanked** — a stale wheel built before the migration-016 fix reached PyPI (which is write-once) and crashed a fresh-DB boot. Use `0.3.8b6`._
