...
Per default lo stack non chiama LLM esterni (embedding locale Granite, parser ingest disattivati). Per attivare:
...
3. Embedding: la ricerca-per-significato usa di default il modello Granite locale (nessuna chiave). Un backend di embedding remoto opzionale, quando installato, si attiva da solo via la sua chiave.
...
Controlla in `.env` di aver impostato almeno una chiave attiva (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY` o `LLM_GATEWAY_*`). L'embedding locale Granite non richiede chiavi. Riavvia con `docker compose up -d --force-recreate api`.
