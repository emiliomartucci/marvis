...
ADMIN_PASSWORD="ci-admin-password-please-change"
ADMIN_PASSWORD_HASH='$2b$12$ex/vdgInlTES.zG6zr6uieY2aNSSNJZ9D2qCDbEkyA7qkQz6QMCCO'
JWT_SECRET="$(head -c 48 /dev/urandom | base64 | tr -dc 'a-zA-Z0-9' | head -c 64)"
ADMIN_EMAIL="ci-test@example.com"
read -r API_PORT CONSOLE_PORT NGINX_PORT < <(python3 - <<'PY'
import socket
...
python3 - "$INSTANCE_DIR/.env" "$ADMIN_PASSWORD" "$ADMIN_PASSWORD_HASH" "$JWT_SECRET" "$PROJECT_NAME" "$ADMIN_EMAIL" "$API_PORT" "$CONSOLE_PORT" "$NGINX_PORT" <<'PY'
import pathlib, sys
env_path = pathlib.Path(sys.argv[1])
admin_password = sys.argv[2]
admin_password_hash = sys.argv[3]
jwt_secret = sys.argv[4]
project = sys.argv[5]
admin_email = sys.argv[6]
overrides = {
    "COMPOSE_PROJECT_NAME": project,
    "API_PORT": sys.argv[7],
    "CONSOLE_PORT": sys.argv[8],
    "NGINX_PORT": sys.argv[9],
    "NEXT_PUBLIC_API_URL": f"http://localhost:{sys.argv[7]}",
    "NEXT_PUBLIC_WS_URL": f"ws://localhost:{sys.argv[7]}",
    "NEXT_PUBLIC_DIRECT_WS_URL": f"ws://localhost:{sys.argv[7]}",
    "NEXT_PUBLIC_DIRECT_WS_PROBE_URL": f"http://localhost:{sys.argv[9]}/healthz",
    "PIR_PASSWORD": admin_password,
    "PIR_ADMIN_PASSWORD_HASH": admin_password_hash,
    "PIR_JWT_SECRET": jwt_secret,
    "PIR_ENV": "development",
    "MARVIS_ADMIN_EMAIL": admin_email,
}
...
for line in lines:
    if "=" in line and not line.lstrip().startswith("#"):
        ...
        if key in overrides:
            out.append(f"{key}={env_value(overrides[key])}")
            seen.add(key)
            continue
    out.append(line)
...
# Wait for nginx readiness. The API is intentionally loopback-only in local
# single-user mode; the template's public readiness endpoint lives on nginx.
API_PORT="$(awk -F= '/^API_PORT=/{print $2; exit}' .env)"
API_PORT="${API_PORT:-8100}"
NGINX_PORT="$(awk -F= '/^NGINX_PORT=/{print $2; exit}' .env)"
NGINX_PORT="${NGINX_PORT:-8080}"
HEALTH_URL="http://localhost:${NGINX_PORT}/healthz"
