...
# v1.15.0 - 2026-03-15 - Add push delivery + Voyage init in lifespan + orphan proxy cleanup
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Console PiR API (env=%s)", settings.pir_env)
    run_migrations()
    ...
    # Load project directories from DB settings
    import json
    import sqlite3
    ...
    from core.api.services.embedding_service import init_voyage_client

    if settings.pir_voyage_disabled:
        logger.info("[canary] skipping Voyage client initialization")
    else:
        init_voyage_client()
