...
def org_encryption_available() -> bool:
    """True when org-scoped provider-key encryption is configured.

    Provider-key creation is fail-closed on a missing ``BYOK_FERNET_SECRET``
    (returns 503). The Console reads this so it can surface the missing-encryption
    state proactively, instead of only on a failed save (gh #22 QA gap)."""
    return bool(settings.byok_fernet_secret)
