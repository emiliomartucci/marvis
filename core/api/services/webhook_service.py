...
# UUID-format branch pattern for PiR task branches
...
async def handle_pr_merged(
    ...
    try:
        ...
        await _send_telegram(
            ...
            f"PR <a href='{pr_html_url}'>#{pr_number}</a> — errore nel merge PiR\n"
...
async def handle_pr_closed(
    ...
    try:
        ...
        await _send_telegram(
            ...
            f"PR <a href='{pr_html_url}'>#{pr_number}</a> — errore nel close PiR\n"
            f"Task: <code>{task_id[:8]}</code>\n"
            f"Errore: {exc}\n"
            f"Repo: {repo}"
        )
