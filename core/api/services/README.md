...
- `remote`: an optional remote embedding backend. It is a deploy-only module
  (`embedding_backends.remote_backend`) and is absent from this build — when
  present and configured it is selected automatically and returns
  512-dimensional vectors; `dual` additionally runs the local engine for
  pre-cutover validation while returning the remote vectors.

Granite-97m R2 is the W1 in-process engine for EU-sensitive ship paths. The decision follows the May 22, 2026 audit at
