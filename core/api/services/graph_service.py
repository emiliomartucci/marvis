...
from core.api.services.kg.manual_edges import list_manual_project_edges
...
def validate_project(project: str | None) -> str | None:
    ...
    if len(project) > MAX_PROJECT_SLUG_LEN:
        raise ValueError(
            ...
            f"Got: {project!r}. "
            f"Fix: use a valid slug from mcp__marvis__list_projects (all slugs are ≤{MAX_PROJECT_SLUG_LEN} chars)."
        )
    if not PROJECT_SLUG_PATTERN.match(project):
        raise ValueError(
            ...
            "Fix: check spelling with mcp__marvis__list_projects; slugs are case-sensitive and must start with [a-z0-9]."
        )
    return project
...
def validate_edge_types(edge_types: list[str] | tuple[str, ...] | None) -> list[str] | None:
    ...
    if not edge_types:
        return None
    unknown = [e for e in edge_types if e not in EDGE_TYPES and e != "related"]
    if unknown:
        raise ValueError(
...
def validate_node_id(node_id: str) -> str:
    ...
    if not isinstance(node_id, str) or not node_id:
        raise ValueError(
            ...
            "Fix: pass a valid node_id string — find one via mcp__marvis__graph_neighbors or SELECT id FROM graph_nodes."
        )
    ...
    if not NODE_ID_PATTERN.match(node_id):
        raise ValueError(
            ...
            "Fix: check prefix+type+slug, or use mcp__marvis__search to find the exact id."
        )
    return node_id
...
# ---------------------------------------------------------------------------
# answer-ready claims (behind MARVIS_KG_CLAIMS) — deterministic, relation-typed
# in-degree counts with capped provenance. SQL lives HERE (the use_case layer is
# SQL-free by contract); the use_case assembles these into KGClaim objects.
# ---------------------------------------------------------------------------

async def count_in_edges_by_relation(
    ...
    *,
    ...
    """Count DISTINCT sources of LIVE in-edges (target=node_id) for one relation.

    In-degree = "how many things depend on / point at node_id". ``valid_until IS
    NULL`` keeps only live edges: counting superseded ones would re-introduce the
    very faithfulness bug this exists to kill. Self-edges excluded.
    ``exclude_sources`` drops specific source ids (meta-hub deny-list).
    """
    sql = (
        "SELECT COUNT(DISTINCT source_id) FROM graph_edges "
        "WHERE target_id = ? AND relation = ? AND valid_until IS NULL "
        "AND source_id != ?"
    )
    params: list[Any] = [node_id, relation, node_id]
    if exclude_sources:
        placeholders = ",".join("?" * len(exclude_sources))
        sql += f" AND source_id NOT IN ({placeholders})"
...
async def sample_in_edge_sources(
    db: aiosqlite.Connection,
    node_id: str,
    relation: str,
    *,
    ...
    """Up to ``limit`` source ids of LIVE in-edges (newest first), capped IN SQL.

    Never fetch the full set and slice in Python: a ``refers_to`` node can have
    100k+ in-edges. Same filters as ``count_in_edges_by_relation``.
    """
...
async def node_trust_fields(
    db: aiosqlite.Connection, node_id: str
) -> dict[str, Any] | None:
    """A claim subject's freshness inputs: ``{type, last_seen_at, last_verified_at}``.

    None if the node is missing. ``last_verified_at`` exists from migration 149
    (ships with the temporal layer alongside the claims flag).
    """
    cur = await db.execute(
        "SELECT type, last_seen_at, last_verified_at FROM graph_nodes "
        "WHERE id = ? LIMIT 1",
        (node_id,),
    )
    row = await cur.fetchone()
...
def _row_to_neighbor(row: aiosqlite.Row, direction: str, edge_relation: str, source_file: str | None, source_line: int | None) -> dict[str, Any]:
    ...
    # at-scale freshness (Fase A): carry raw timestamps ONLY when the caller
    # selected them (with_freshness=True). The use-case layer turns them into
    # needs_review/derivation and strips this block; flag-off never sees it.
    if "node_updated_at" in row.keys():
        neighbor["freshness"] = {
            "node_updated_at": row["node_updated_at"],
            "edge_last_seen_at": row["edge_last_seen_at"],
            # Fase D: a Brain REINFORCE (Triage-approved) stamps last_verified_at;
            # carried here so the read-time needs_review treats a re-verified node
            # as the freshest signal. NULL when never verified (mig 149 default).
            "node_last_verified_at": (
                row["node_last_verified_at"]
                if "node_last_verified_at" in row.keys() else None
            ),
        }
    return neighbor


async def _manual_project_neighbors(
    db: aiosqlite.Connection,
    *,
    workspace_id: str,
    node_id: str,
    ...
    if as_of is not None or not node_id.startswith("project:artifact:"):
        return []

    cur = await db.execute(
        "SELECT name FROM graph_nodes WHERE id = ? AND type = 'project' LIMIT 1",
        (node_id,),
    )
    ...
    edges = await list_manual_project_edges(db, workspace_id=workspace_id)
    neighbor_slugs: list[tuple[str, str, str]] = []
    for edge in edges:
        if wanted is not None and edge.kind not in wanted:
            continue
        if direction in ("incoming", "both") and edge.dst_slug == slug:
            neighbor_slugs.append((edge.src_slug, "incoming", edge.kind))
        if direction in ("outgoing", "both") and edge.src_slug == slug:
            neighbor_slugs.append((edge.dst_slug, "outgoing", edge.kind))

    if not neighbor_slugs:
        return []

    out: list[dict[str, Any]] = []
    for neighbor_slug, edge_direction, edge_relation in neighbor_slugs:
        if project is not None and neighbor_slug != project:
            continue
        cur = await db.execute(
            "SELECT id, type, name, qualified_name, file_path, line_number, metadata "
            "FROM graph_nodes WHERE type = 'project' AND name = ? LIMIT 1",
            (neighbor_slug,),
        )
        row = await cur.fetchone()
        if row is None:
...
async def get_neighbors(
    ...
    if edge_types is not None:
        ...
        if relation == "related":
            manual_relation_only = True
            where_relation = " AND 1 = 0"
        elif relation not in EDGE_TYPES:
            raise ValueError(
    ...
    if edge_types is not None and all(edge_type == "related" for edge_type in edge_types):
        manual_relation_only = True
        where_relation = " AND 1 = 0"
        relation_params = []
    ...
    # at-scale freshness (Fase A): opt-in extra columns for the read-time
    # needs_review/derivation computation. Default off → SQL byte-identical.
    fresh_cols = (
        ", n.updated_at AS node_updated_at, e.last_seen_at AS edge_last_seen_at"
        ", n.last_verified_at AS node_last_verified_at"
        if with_freshness else ""
    )
    ...
    if direction in ("incoming", "both") and not manual_relation_only:
    ...
    if direction in ("outgoing", "both") and not manual_relation_only:
        sql = (
            "SELECT n.id, n.type, n.name, n.qualified_name, n.file_path, "
            "n.line_number, n.metadata, e.relation AS edge_relation, "
            "e.source_file AS edge_source_file, e.source_line AS edge_source_line"
    ...
    manual_results = await _manual_project_neighbors(
        db,
    ...
    return (manual_results + results)[:limit]
...
async def graph_context(
    ...
    async def _project_nodes_by_type(
        project_slug: str, node_type: str
    ) -> list[dict[str, Any]]:
        cur = await db.execute(
            """
            SELECT id, type, name, qualified_name, file_path, line_number, metadata
            ...
            WHERE project_id = ?
              AND type = ?
              AND deprecated_at IS NULL
            ORDER BY COALESCE(updated_at, created_at) DESC, qualified_name, name, id
            LIMIT ?
            """,
            (project_slug, node_type, per_category_limit),
        )
        rows = await cur.fetchall()
        return [_node_row_to_dict(r) for r in rows]

    if node_dict.get("type") == "project":
        project_slug = (
            node_dict.get("metadata", {}).get("slug")
            or node_id.split(":", 2)[2]
        )
        commits = await _project_nodes_by_type(project_slug, "commit")
        prs = await _project_nodes_by_type(project_slug, "pr")
        tasks = await _project_nodes_by_type(project_slug, "task")
        handoffs = await _project_nodes_by_type(project_slug, "handoff")
        learnings = await _project_nodes_by_type(project_slug, "learning")
        return {
            "node": node_dict,
            "commits": commits,
            "prs": prs,
