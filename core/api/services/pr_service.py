...
def _resolve_register_worktree_path(
    *, repo_path: str, worktree_path: str | None
) -> str:
    """Resolve and validate the worktree path persisted for a PR draft.

    Hosted MCP runs on the hosted server. A path that exists on a local/Hetzner
    machine is poison here: later submit/merge calls see an empty or misleading
    diff. If the caller omits a path, use the project repo itself; if they pass
    one, require it to be a hosted-visible git worktree root.
    """
    candidate = Path(worktree_path or repo_path).expanduser()
    try:
        resolved = candidate.resolve(strict=True)
    except OSError as exc:
        raise ValueError(
            f"Worktree path is not accessible on this server: {candidate}"
        ) from exc

    git_marker = resolved / ".git"
    if not resolved.is_dir() or not git_marker.exists():
        raise ValueError(
            "Worktree path must be a git repository visible on this server: "
            f"{resolved}"
        )
    return str(resolved)
...
async def _fetch_github_labels(repo_path: str, branch: str) -> list[str]:
    """Fetch labels for a GitHub PR branch via gh CLI when available."""
    if shutil.which("gh") is None:
        logger.debug("gh CLI unavailable; skipping GitHub labels for branch %s", branch)
        return []
...
def _is_missing_pr_branch_error(exc: Exception) -> bool:
    """Whether diff failed because a historical PR branch is absent locally."""
    message = str(exc)
...
async def get_merge_conflicts(
    ...
    for row in rows:
        try:
            ...
            if _is_missing_pr_branch_error(exc):
                logger.debug(
                    "get_merge_conflicts: skipping stale PR task %s branch %s: %s",
                    row["task_id"],
                    row["branch"],
                    exc,
                )
...
async def _check_main_in_sync(repo_path: str) -> None:
    ...
    """
    if not git_ops.git_remote_sync_enabled():
        logger.info(
            "Skipping origin/main sync check because MARVIS_GIT_PUSH_ON_MERGE=false"
        )
        return
...
async def register_branch(
    ...
    *,
    ...
    repo_path = await _get_repo_path(db, project)
    resolved_worktree_path = _resolve_register_worktree_path(
        repo_path=repo_path,
...
async def merge_pr(
    ...
    *,
    ...
    # Cleanup orchestrator-created worktree (best effort). Manually registered PRs
    # may not have a worktree_path, and there is nothing to remove in that case.
    if pr["worktree_path"]:
        try:
