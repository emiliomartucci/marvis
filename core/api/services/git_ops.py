...
def _push_on_merge() -> bool:
    """Whether PR merge should sync with/push to origin."""
    return _env_flag("MARVIS_GIT_PUSH_ON_MERGE", default=True)


def git_remote_sync_enabled() -> bool:
    """Whether hosted git operations may contact origin.

    Hosted local-only tenants set ``MARVIS_GIT_PUSH_ON_MERGE=false`` because
    their canonical repo lives in the tenant filesystem. In that mode every
    origin check is advisory at best and a hard blocker when no deploy key is
    configured, so callers must skip remote sync gates.
    """
    return _push_on_merge()


def _merge_commit_env() -> dict[str, str]:
    """Git env with a deterministic committer identity for service accounts."""
    env = os.environ.copy()
    name = env.get("MARVIS_GIT_COMMITTER_NAME", "Marvis Hosted")
    email = env.get("MARVIS_GIT_COMMITTER_EMAIL", "marvis@localhost")
...
def _merge_branch_sync(
    ...
    if push_on_merge:
        ...
        logger.info("Skipping origin sync before merge because MARVIS_GIT_PUSH_ON_MERGE=false")
    ...
    try:
        # Always create a merge commit (--no-ff), even when a fast-forward is
        # possible. A pure fast-forward leaves the PR's commits LINEAR on the
        # target, invisible to the merge-based release-notes generator
        # (`core/scripts/release-notes.sh` walks `git log --merges`). A discrete
        # merge commit per PR is the anchor it categorizes the branch's
        # conventional commits under. The diverged case was already a merge
        # commit (3-way); this makes the fast-forward case match.
        merge_args = ["merge", "--no-ff", "--no-edit", "--no-verify", branch]
        ...
        if push_on_merge:
            ...
            logger.info("Skipped push to origin/%s because MARVIS_GIT_PUSH_ON_MERGE=false", target)
