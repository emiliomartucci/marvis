...
async def _project_colors(
    ...
    *,
    ...
    params: list[str] = [workspace_id, *sorted(slugs)]
    try:
        cur = await db.execute(
            "SELECT project_slug, color FROM project_gui_metadata "
            f"WHERE workspace_id = ? AND project_slug IN ({placeholders})",
            params,
        )
...
async def fetch_cosmo_graph(
    ...
    try:
        ...
        manual_edge_rows = await list_manual_project_edges(
            db,
        ...
        # BaseException, not Exception: a cancelled request (CancelledError) must
        # still end the DEFERRED transaction, otherwise the pooled read
        # connection returns to the pool pinned to a stale WAL snapshot and every
        # later borrower reads a frozen view. (task 6ba40024, learning 4b80fdfb)
        await db.rollback()
        raise
    ...
    visible_project_slugs = {
        str(r["slug"])
        for r in project_rows
        if visible is None or str(r["slug"]) in visible
    }
    colors = await _project_colors(
        db,
        workspace_id=user.workspace_id or "ws_default",
        slugs=visible_project_slugs,
    )
    ...
    project_slugs = {p.slug for p in projects_out}
    edges_out: list[Edge] = []
    for r in edge_rows:
        a: str = r["slug_a"]
        b: str = r["slug_b"]
        if a not in project_slugs or b not in project_slugs:
            continue
    ...
    for edge in manual_edge_rows:
        if edge.src_slug not in project_slugs or edge.dst_slug not in project_slugs:
            continue
        if visible_slugs is not None and (
            edge.src_slug not in visible_slugs or edge.dst_slug not in visible_slugs
        ):
            continue
        edges_out.append(
            Edge(
                source=edge.src_slug,
                target=edge.dst_slug,
                relation=edge.kind,
                weight=1.0,
            )
        )
