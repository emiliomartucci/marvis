...
"""Hybrid search service — fuse semantic (Voyage+sqlite-vec) with multiple FTS5 sources.
...
Voyage and the four SQLite FTS5 indices never block each other. Each
...
# "iperammortamento" then revealed that Voyage cosine was handing a score
...
#
# Total = 0.30 + 0.25 + 0.25 + 0.10 + 0.10 = 1.00 (gated by
...
async def kg_full_text_search(
    ...
    try:
        ...
        try:
            ...
            msg = str(exc).lower()
            if "no such table" in msg or "syntax" in msg:
...
async def _run_semantic_branch(
    ...
    try:
        ...
        if not embedding_service.is_available():
            return None, RuntimeError("voyage-unavailable")
    ...
    if sys.version_info >= (3, 11):
        async with asyncio.TaskGroup() as tg:
            sem_task = tg.create_task(
                _run_semantic_branch(q, workspace_id, db_path, vec0_path, fetch_n)
            )
        ...
        sem_result, sem_err = await _run_semantic_branch(
...
def _suggest_empty(
    *,
    ...
    if sem_ok and kg_ok and tasks_ok and inbox_ok and learn_ok:
        out.extend(
            [
