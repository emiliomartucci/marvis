...
async def compensate_step(
    ...
    async with acquire_write_db() as db:
        ...
        elif step_failed == "remote_embed":
            # Remote-embed cleanup lives in a deploy-only helper that is absent
            # in the OSS build — degrade cleanly when it is not installed.
            try:
                from core.api.services.ingest.remote_embed_retry import (
                    enqueue_remote_embed_removal,
                )
            except ImportError:
                logger.info(
                    "remote-embed compensation helper absent; skipping cleanup for %s",
                    ingest_id,
                )
            else:
                await enqueue_remote_embed_removal(ingest_id)
        await db.execute(
            """
            UPDATE ingest_pending
               SET status = 'parse_error',
