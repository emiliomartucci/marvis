...
def _marvisx_tmux_tmpdir() -> str:
    return os.path.join(f"/run/user/{os.getuid()}", MARVISX_TMUX_TMPDIR_NAME)
...
def _scoped_tmux_new_session_command(*args: str) -> tuple[str, ...]:
    ...
    return (
        ...
        f"--setenv=XDG_RUNTIME_DIR=/run/user/{os.getuid()}",
...
async def _list_sessions_for_server(server: TmuxServer) -> list[dict]:
    ...
    if returncode != 0:
        ...
        if "no server running" in stderr_text or "no sessions" in stderr_text:
            return []
        if "error connecting" in stderr_text and "No such file" in stderr_text:
            return []
        logger.error("tmux list-sessions failed for %s: %s", server, stderr_text)
        return []
