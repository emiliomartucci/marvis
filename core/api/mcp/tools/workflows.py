...
async def _save_doc_artifact(
    ...
    *,
    ...
    """Write a workflow artifact under ``<project>/docs/<subdir>/`` + embed best-effort.
    ...
    just-written artifact is immediately findable by meaning (B2 ⇽ A1). The embed is
    best-effort and runs OUTSIDE any writer lock (it is its own ``write_db``
    acquisition inside ``embed_doc_document``): a failed embed NEVER fails the save —
    worst case the artifact is keyword/FTS-only until a reindex, the same fallback
    floor A1 documents. Returns ``{path, embedded}``.
    ...
    return {"path": str(doc_path), "embedded": embedded}
...
def register(mcp) -> None:
    ...
    async def save_plan(
        ...
        RESTITUISCE: {path, embedded} — il percorso del file scritto e se l'embedding e' andato a buon fine (best-effort: il salvataggio riesce anche se l'embed fallisce)."""
        try:
