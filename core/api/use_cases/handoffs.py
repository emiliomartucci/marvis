...
_HANDOFF_SUFFIX_RE = re.compile(r"[^a-z0-9]+")
...
def _slugify_handoff_suffix(value: str | None) -> str:
    """Return a filename-safe suffix for generated handoff names."""
    raw = (value or "session").strip().lower()
    slug = _HANDOFF_SUFFIX_RE.sub("-", raw).strip("-")
    return slug[:60].strip("-") or "session"


def _normalise_tags(tags: list[str] | None) -> list[str]:
    """Normalise user-provided frontmatter tags preserving order."""
    out: list[str] = []
    seen: set[str] = set()
    for raw in tags or []:
        tag = str(raw).strip()
...
def _render_handoff_content(frontmatter: dict, body: str) -> str:
    fm = yaml.safe_dump(
        frontmatter,
        allow_unicode=True,
        sort_keys=False,
...
async def create_handoff(
    ...
    *,
    ...
    """Create a project-scoped handoff markdown file.

    The caller provides the markdown body without YAML frontmatter. The use case
    ...
    if not body_clean:
        raise ServiceError(code="invalid_body", message="Handoff body is required")
    if body_clean.startswith("---"):
        raise ServiceError(
            code="invalid_body",
            message="Pass markdown body only; create_handoff owns YAML frontmatter",
        )

    handoff_date = date or _date.today().isoformat()
    try:
        _date.fromisoformat(handoff_date)
    ...
    if filename is None:
        suffix = _slugify_handoff_suffix(title or summary)
        filename = f"handoff-{handoff_date}-{suffix}.md"
    ...
    try:
        ...
        if not str(memory_dir).startswith(str(project_root) + "/"):
            raise ServiceError(code="invalid_project_path", message="Invalid project path")
    except ServiceError:
        raise
    ...
    if session not in (None, ""):
        frontmatter["session"] = str(session)
    if branch:
        frontmatter["branch"] = branch
    normalised_tags = _normalise_tags(tags)
    if normalised_tags:
        frontmatter["tags"] = normalised_tags
    if task_id:
        frontmatter["task_id"] = task_id

    sections: list[str] = []
    if title:
        sections.append(f"# Handoff - {title.strip()}")
    if summary:
...
async def reindex_handoffs(
    ...
    *,
    ...
    it only guards on embedding availability (503 when the embedding backend is not configured).
    """
    from core.api.services import embedding_service
