...
def _git_head_commit_at(repo_path) -> datetime | None:
    """Best-effort HEAD commit timestamp for freshness checks."""
    import shutil
    import subprocess
    ...
    try:
        ...
        return None
    return _parse_graph_datetime(out.stdout.strip())
...
async def compute_index_freshness(
    ...
    # last_modified_git_sha is file provenance; last_seen_at/touch_last_at
    # are index freshness. Full rebuilds refresh last_seen_at even when the
    # file provenance SHA stays behind a merge commit.
    freshness_ts_expr = (
        "CASE "
        "WHEN last_seen_at IS NULL THEN touch_last_at "
        "WHEN touch_last_at IS NULL THEN last_seen_at "
        "WHEN last_seen_at >= touch_last_at THEN last_seen_at "
        "ELSE touch_last_at END"
    )
    cur = await db.execute(
        "SELECT COUNT(*) AS total, "
        "       COUNT(NULLIF(last_modified_git_sha, '')) AS with_sha, "
        f"       MAX({freshness_ts_expr}) AS last_indexed "
    ...
    cur = await db.execute(
        ...
        f"AND last_modified_git_sha != '' ORDER BY {freshness_ts_expr} DESC LIMIT 1",
        (proj,),
    )
    ...
    if with_sha == 0 or not indexed_sha:
        ...
        # ``last_modified_git_sha`` is file provenance, not the project-index run
        # marker. A merge commit can be HEAD without touching a given KG node.
        # If the index timestamp is newer than HEAD, the slice has been refreshed.
        head_commit_at = _git_head_commit_at(repo_path)
        last_indexed_at = _parse_graph_datetime(result.get("last_indexed_at"))
        if head_commit_at is not None and last_indexed_at is not None and last_indexed_at >= head_commit_at:
            result["reason"] = "fresh_by_index_time"
        else:
...
def _neighbor_summary(
    ...
    *,
    ...
    for n in raw:
        ...
        rel = n.get("relation") or edge.get("relation")
        if rel:
            by_relation[rel] += 1
        direction = n.get("direction") or edge.get("direction")
        if direction:
...
async def graph_resolve(
    ...
    *,
    ...
    db.row_factory = aiosqlite.Row
    row = None
    for candidate in _graph_resolve_candidate_paths(path):
        cur = await db.execute(
            """
...
def _graph_resolve_candidate_paths(path: str) -> list[str]:
    candidates = [path]
    try:
        from core.api.routers.projects import PROJECT_DIRS
    except Exception:  # noqa: BLE001 - keep resolve fail-closed on import-time wiring issues
        return candidates
