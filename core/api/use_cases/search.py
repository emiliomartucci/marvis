...
async def search(
    ...
    *,
    ...
    if hybrid:
        ...
        # U5: a truly empty result over an unbuilt index → kick a self-healing
        # background build and tell the user it is building (retry shortly).
        if not any(grouped.values()):
            if await _maybe_kick_lazy_build(workspace_id):
                meta = {
                    **(meta or {}),
                    "semantic_available": False,
                    "semantic_reason": "index-building",
                }
