...
def test_neighbor_summary_counts_nested_edge_shape():
    raw = [
        {"id": "a", "edge": {"relation": "depends_on", "direction": "incoming"}},
        {"id": "b", "edge": {"relation": "mentions", "direction": "outgoing"}},
        {"id": "c", "edge": {"relation": "mentions", "direction": "outgoing"}},
    ]

    s = _neighbor_summary(raw)

    assert s["total"] == 3
    assert s["distinct_nodes"] == 3
    assert s["by_relation"] == {"mentions": 2, "depends_on": 1}
    assert s["by_direction"] == {"incoming": 1, "outgoing": 2}
