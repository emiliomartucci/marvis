...
def _run_sync(repo_dir: Path, merge_sha: str, pre_merge_sha: str) -> None:
    asyncio.run(
        workspace_sync.sync_primary_workspace_after_merge(
            repo_dir, merge_sha, pre_merge_sha, target_branch="main"
        )
    )
...
def test_sync_pr_branch_checkout_is_not_treated_as_user_dirty(
    repo: Path,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """A clean PR branch checkout should not trigger workspace_sync_partial."""
    (repo / "service.py").write_text("v1\n")
    _git(repo, "add", "service.py")
    _git(repo, "commit", "-m", "add service")
    pre = _git(repo, "rev-parse", "main").stdout.strip()

    merge_sha = _make_commit_on_branch(repo, "service.py", "v2\n", "bump service")
    _git(repo, "checkout", "feat")
    _git(repo, "update-ref", "refs/heads/main", merge_sha)

    _run_sync(repo, merge_sha, pre)

    assert (repo / "service.py").read_text() == "v2\n"
    assert not any("workspace_sync_partial" in rec.message for rec in caplog.records)


def test_sync_pr_branch_checkout_preserves_staged_user_edits(
    repo: Path,
) -> None:
    """Staged edits on a PR branch are still protected."""
    (repo / "service.py").write_text("v1\n")
    _git(repo, "add", "service.py")
    _git(repo, "commit", "-m", "add service")
    pre = _git(repo, "rev-parse", "main").stdout.strip()
    ...
    (repo / "service.py").write_text("user staged\n")
    _git(repo, "add", "service.py")

    _run_sync(repo, merge_sha, pre)

    assert (repo / "service.py").read_text() == "user staged\n"
    staged = _git(repo, "diff", "--cached", "--name-only").stdout
    assert "service.py" in staged


def test_sync_pr_branch_checkout_preserves_unstaged_user_edits(repo: Path) -> None:
    """Unstaged edits on a PR branch are still protected."""
    (repo / "service.py").write_text("v1\n")
    _git(repo, "add", "service.py")
    _git(repo, "commit", "-m", "add service")
    pre = _git(repo, "rev-parse", "main").stdout.strip()
    ...
    (repo / "service.py").write_text("user unstaged\n")

    _run_sync(repo, merge_sha, pre)

    assert (repo / "service.py").read_text() == "user unstaged\n"
