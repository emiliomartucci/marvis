...
async def _db(rows):
    ...
    for r in rows:
        ...
        if len(row) == 4:
            row = (*row, row[3])
        await conn.execute(
            "INSERT INTO graph_nodes (id, project_id, last_modified_git_sha, touch_last_at, last_seen_at) "
            "VALUES (?, ?, ?, ?, ?)",
            row,
        )
    await conn.commit()
...
@pytest.mark.asyncio
async def test_fresh_when_last_seen_ran_after_head_even_if_touch_is_old(monkeypatch):
    monkeypatch.setattr("core.api.routers.projects._find_git_path", lambda slug: "/tmp/repo")
    monkeypatch.setattr(graph_mod, "_git_head_sha", lambda repo: "newsha9")
    monkeypatch.setattr(
        graph_mod,
        "_git_head_commit_at",
        lambda repo: datetime(2026, 5, 2, tzinfo=timezone.utc),
    )
    db = await _db(
        [
            (
    ...
    try:
        out = await compute_index_freshness(db, node_id="py:function:x.y", project="alpha")
        assert out["reason"] == "fresh_by_index_time"
        assert out["stale"] is False
        assert out["last_indexed_at"] == "2026-05-03T00:00:00+00:00"
    finally:
        await db.close()

@pytest.mark.asyncio
async def test_fresh_when_index_ran_after_head_even_if_file_sha_differs(monkeypatch):
    monkeypatch.setattr("core.api.routers.projects._find_git_path", lambda slug: "/tmp/repo")
    monkeypatch.setattr(graph_mod, "_git_head_sha", lambda repo: "newsha9")
    monkeypatch.setattr(
        graph_mod,
    ...
    db = await _db([("py:function:x.y", "alpha", "oldsha1", "2026-05-03T00:00:00+00:00")])
    try:
        out = await compute_index_freshness(db, node_id="py:function:x.y", project="alpha")
        assert out["reason"] == "fresh_by_index_time"
        assert out["stale"] is False
