...
try:
    import tomllib
except ModuleNotFoundError:  # pragma: no cover - Python 3.10 installed wheels use package metadata.
...
def _pyproject_cli_version() -> str:
    ...
    try:
        ...
        logger.debug("Failed to read pyproject version fallback", exc_info=True)
        return "0.0.0"
...
def _installed_cli_version() -> str:
    try:
        ...
        return _pyproject_cli_version()
    except Exception:  # noqa: BLE001
        logger.exception("Failed to read installed package version")
        return "0.0.0"


def _read_cached_latest_version() -> str | None:
    try:
        if _UPDATE_LATEST_CACHE.is_file():
            return _UPDATE_LATEST_CACHE.read_text(encoding="utf-8").strip() or None
    except Exception:  # noqa: BLE001
        logger.debug("Failed to read cached latest version", exc_info=True)
    return None
...
def _is_newer_version(latest: str | None, installed: str) -> bool:
    if latest is None:
        return False
    try:
...
async def get_version() -> dict[str, str | bool | None]:
    """Installed CLI version + cached update notice. No network call here."""
    installed = _installed_cli_version()
    latest = _read_cached_latest_version()
    return {
        "installed": installed,
        "latest": latest,
