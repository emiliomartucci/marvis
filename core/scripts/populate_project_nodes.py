...
# Single source for the project super-node upsert — used by the full populator
# AND the lightweight seed (gh #35), so the two never drift.
_PROJECT_NODE_UPSERT_SQL = """
...
def _project_node_metadata(info: dict[str, Any]) -> str:
    """Serialize the project node metadata blob (kept identical to the populator)."""
    return json.dumps(
        {
...
def seed_project_nodes_only(
    db: str, projects_root: Path = DEFAULT_PROJECTS_ROOT
) -> int:
    """Upsert ONLY the project super-nodes (no aggregate edges) for every project
    discovered under ``projects_root``. Returns the count upserted.

    Lightweight backfill so Universe/Cosmo shows registered projects even before
    code indexing (gh #35): the registration of a project never created a KG node
    (only indexing did), so a fresh local install had a populated project list but
    an empty graph. The full :func:`populate_project_nodes` also recomputes
    cross-project edges (expensive, needs indexed code) — overkill just to make the
    project bubbles appear. Idempotent (upsert); best-effort per project so one bad
    project.yaml never blocks the rest.
    """
    ...
    try:
        ...
        for slug, info in projects.items():
            ...
            try:
                ...
                conn.execute(
                    _PROJECT_NODE_UPSERT_SQL,
                    (node_id, slug, f"project.{slug}", _project_node_metadata(info), slug),
                )
                conn.commit()
                written += 1
                ...
                conn.rollback()
                logger.warning(
                    "seed_project_nodes_only: upsert failed for %s", slug, exc_info=True
                )
        return written
    finally:
        conn.close()
...
def populate_project_nodes(
    ...
    try:
        ...
        for slug, info in projects.items():
            node_id = _project_node_id(slug)
            metadata = _project_node_metadata(info)
