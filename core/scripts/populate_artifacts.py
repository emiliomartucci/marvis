...
PROJECTS_ROOT = LEGACY_MANAGED_PROJECTS_ROOT


def _projects_root() -> Path:
    return resolve_projects_root(PROJECTS_ROOT)


def _default_metadata_path() -> Path:
    return _projects_root() / DEFAULT_PROJECT


DEFAULT_METADATA_PATH = _default_metadata_path()
...
def _route_metadata_path(
    ...
    non e' indicizzabile sotto il projects root configurato.
    ...
    """
    projects_base = projects_root or _projects_root()
    try:
        resolved = path.resolve() if path.exists() else path.absolute()
        rel = resolved.relative_to(projects_base)
    ...
    if len(parts) < 2:
        return None
    slug = parts[0]
    metadata_path = projects_base / slug
    # context.md
...
def _index_handoff(
    ...
    if task_id not in valid_task_ids:
        return {"skipped": {
            ...
            "task_id": task_id,
            "fix_hint": "verify task exists in Marvis and re-run populate_tasks_and_prs",
        }}
...
def _soft_delete_artifact(
    ...
    if kind == "handoff":
        ...
        try:
            route = _route_metadata_path(path, projects_root=projects_root)
...
def populate_artifacts_incremental(
    ...
    Args:
        paths: lista di file da indicizzare (assoluti, tipicamente
               `<projects-root>/<slug>/memory/handoff-*.md` o
               `<slug>/docs/<type>/*.md`).
        ...
        projects_root: override root metadata (default: configurazione/env).
        ...
        for raw in paths:
            ...
            if route is None:
                ...
                results["skipped"].append({
                    ...
                    "reason": "unroutable",
                    "fix_hint": (
                        "path must be under <projects-root>/<slug>/memory/handoff-*.md "
            ...
            if handle_delete or not p.exists():
                del_out = _soft_delete_artifact(conn, p, kind, projects_root=base)
...
def _main() -> int:
    ...
    ap.add_argument(
        "--metadata-path",
        default=None,
        help=f"Project metadata path (default: <projects-root>/{DEFAULT_PROJECT})",
    )
    ap.add_argument(
        "--projects-root",
        default=None,
    ...
    ap.add_argument(
        ...
        help="Process N paths in a single invocation instead of a full scan. "
             "Each PATH must be under <projects-root>/<slug>/ (memory/, docs/, "
    ...
    ap.add_argument(
        ...
        help="Phase 6: dopo il run --project=marvisx (o al posto), indicizza "
             "handoff + knowledge_docs per ogni slug in <projects-root>/<slug>/ "
             "con project.yaml valido. commits/tasks/PRs NON "
    ...
    out = populate_artifacts(
        ...
        metadata_path=Path(args.metadata_path) if args.metadata_path else projects_root / args.project,
        since_days=args.since_days,
        max_commits=args.max_commits,
