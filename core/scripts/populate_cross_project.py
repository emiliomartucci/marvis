...
from core.scripts._projects_root import (
...
DEFAULT_PROJECTS_ROOT = LEGACY_MANAGED_PROJECTS_ROOT


def _projects_root() -> Path:
    return resolve_projects_root(DEFAULT_PROJECTS_ROOT)
...
# (CI + cross-program pilot + core hub) con projects root configurato.
...
try:
    ACTIVE_SLUGS: frozenset[str] = _discover_all_slugs(_projects_root())
...
def _path_storage_candidates(abs_path: str, projects_root: Path) -> list[str]:
    """Return file_path variants used by older KG artifact populators."""
    candidates = [abs_path]
    path = Path(abs_path)
    try:
        candidates.append(str(Path("projects") / path.relative_to(projects_root)))
    except ValueError:
        pass
    try:
        candidates.append(str(path.relative_to(projects_root.parent)))
    except ValueError:
        pass
...
def _extract_prose_edges(
    ...
    for item in parsed:
        ...
        # Check relative-to-projects-root form:
        # <projects-root>/marvisx/memory/x.md
        # populate_artifacts stored as projects/marvisx/memory/x.md for marvisx.
        for candidate in _path_storage_candidates(abs_path, projects_root):
...
def _compute_similar_to(
    ...
    # documents.file_path invece e' SEMPRE ASSOLUTO.
    ...
    for row in candidates:
        ...
        if file_path.startswith("projects/"):
            # Convert relative 'projects/X/...' → absolute
            # '<projects-root>/X/...'.
            candidate_paths.append(str(projects_root.parent / file_path))
        doc = None
        for cp in candidate_paths:
            doc = conn.execute(
...
def populate_cross_project(
    ...
    base = projects_root or _projects_root()
    ...
    try:
        ...
        if skip_prose:
            ...
            n = _extract_prose_edges(conn, projects, projects_root=base, workers=workers)
        ...
        if skip_similar_to:
            ...
            n = _compute_similar_to(conn, projects, projects_root=base)
...
def _resolve_db_path(explicit: str | None = None) -> str:
    ...
    if explicit_env:
        return str(Path(explicit_env).expanduser())

    prod = Path("/data/pir/console.db")
    return str(prod if prod.exists() else REPO_ROOT / "console.db")
...
def _resolve_md_src_id(
    ...
    rel_candidates = _path_storage_candidates(abs_path, projects_root)[1:]
    ...
    project_id = "marvisx"
    try:
        parts = Path(abs_path).resolve().relative_to(projects_root).parts
        if parts:
            project_id = parts[0]
...
def _extract_prose_edges_for_file(
    ...
    src_id, src_new_node = _resolve_md_src_id(conn, abs_path, projects_root)
    if not src_id:
        return [], []
...
def _resolve_cross_project_src_id(
    ...
    base = projects_root or _projects_root()
    for candidate in _path_storage_candidates(abs_path, base):
        row = conn.execute(
            "SELECT id FROM graph_nodes WHERE file_path=? LIMIT 1",
            (candidate,),
        ).fetchone()
...
def populate_cross_project_incremental(
    ...
    try:
        ...
        for raw in paths:
            p = Path(raw)
            route = _route_metadata_path(p, projects_root=base)
            if route is None:
                results["files_skipped_unroutable"] += 1
                results["skipped"].append({
                    "file": str(p),
                    "reason": "unroutable",
                    "fix_hint": "path must be under <projects-root>/<slug>/",
                })
                continue
            ...
            if handle_delete or not p.exists():
                node_id = _resolve_cross_project_src_id(
                    conn,
                    abs_path,
                    kind=kind,
                    projects_root=base,
            ...
            src_id, _ = _resolve_md_src_id(conn, abs_path, base)
            if src_id:
                conn.execute("BEGIN IMMEDIATE")
                try:
                    conn.execute(
...
def _main() -> int:
    ...
    ap.add_argument("--projects-root", default=None,
                    help=f"Root path for project dirs (default: {_projects_root()})")
