...
def create_pir_task(check_name: str, drift_detail: dict[str, object]) -> str | None:
    """Create a PiR task via curl and Bearer auth; return the task id if created."""
...
def dedup_check(
    ...
    *,
    ...
    """Race-safe dedup insert; create one PiR task for a new active drift."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Only post-commit records drift to docs_drift_history and PiR.",
    )
    parser.add_argument(
        "--db-path",
