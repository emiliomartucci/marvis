...
def check_bash_merge(
    ...
    return _deny(
        ...
        "Fix: from a feat/task-{task_id} worktree → commit → `mcp__marvis__submit_pr(task_id, title, body)` → an admin approves in Triage (your console) → merge happens via console cookie-auth. "
...
def check_subtree_push(
    ...
    return _deny(
        ...
        "Fix: submit the PR (`mcp__marvis__submit_pr`), let an admin merge it via Triage, THEN run `bash scripts/deploy-from-ref.sh origin/main all` from main. "
    ...
    try:
        ...
        print(
            _hook_response(
                ...
                "If the crash is deterministic, file a Marvis task before attempting to bypass."
            )
        )
        return 0
