...
# --------------------------------------------------------------------------- #
# Org BYOK encryption master secret (gh #36)
#
# This is the master secret consumed by core.api.services.crypto for the
# org-scoped provider keys behind the Console "Add provider key" flow
# (BYOK_FERNET_SECRET). It is INDEPENDENT of the byok.vault master key above:
# that one encrypts the on-disk provider-key file store; this one is the HKDF
# input keying material for the DB provider_keys table. Storing it lets a fresh
# local install encrypt provider keys out-of-the-box instead of failing closed
# (503) until an admin exports BYOK_FERNET_SECRET by hand.
# --------------------------------------------------------------------------- #

BYOK_FERNET_SECRET_FILENAME = "byok_fernet.key"


def ensure_local_byok_fernet_secret(vault_dir: Path | None = None) -> str:
    """Generate-and-persist (once) the local-tier BYOK encryption master secret.

    Returns the secret. Idempotent: an existing non-empty key file is returned
    untouched. Persisted as a 0600 file in the vault (``~/.marvis`` or
    ``$MARVIS_VAULT_DIR``). Managed deploys set ``BYOK_FERNET_SECRET`` via env and
    never call this — env always wins. The value is high-entropy random keying
    material (HKDF input in crypto.py), not a passphrase, so a raw token is the
    right primitive (the per-key Fernet keys are HKDF-derived downstream).
    """
    base = (vault_dir or Path(os.environ.get("MARVIS_VAULT_DIR") or DEFAULT_VAULT_DIR)).expanduser()
    path = base / BYOK_FERNET_SECRET_FILENAME
    try:
    ...
    secure_path(base, mode=0o700)
    secret = secrets.token_urlsafe(48)
    tmp = base / (BYOK_FERNET_SECRET_FILENAME + ".tmp")
    tmp.write_text(secret + "\n", encoding="utf-8")
    _chmod_600(tmp)
    os.replace(tmp, path)
    _chmod_600(path)
    logger.info("Provisioned local BYOK encryption secret at %s", path)
    return secret
