...
def _tenant_id_from_env() -> str | None:
    for key in ("MARVIS_INSTANCE", "PIR_INSTANCE", "TENANT_ID"):
...
def _check_tenant_resolved_paths(tenant: str) -> CheckResult:
    db = os.environ.get("MARVIS_DB_PATH") or os.environ.get("PIR_DB_PATH")
    projects = os.environ.get("MARVIS_PROJECTS_ROOT") or os.environ.get("WORKSPACE_ROOT")
    detail = f"tenant={tenant} | db={db or '<unset>'} | projects={projects or '<unset>'}"
    missing = []
    if not db:
        missing.append("MARVIS_DB_PATH/PIR_DB_PATH")
    if not projects:
        missing.append("MARVIS_PROJECTS_ROOT/WORKSPACE_ROOT")
    if missing:
        return CheckResult(
            name="tenant_resolved_paths",
            level="error",
            detail=f"missing tenant env: {', '.join(missing)}. {detail}",
            fix="source /etc/marvis/tenants/<tenant>/env before running marvis doctor",
        )
    if not Path(db).is_absolute() or not Path(projects).is_absolute():
        return CheckResult(
            name="tenant_resolved_paths",
            level="error",
            detail=f"tenant paths must be absolute. {detail}",
            fix="repair /etc/marvis/tenants/<tenant>/env with absolute DB and projects paths",
        )
    return CheckResult(name="tenant_resolved_paths", level="ok", detail=detail)


def _systemctl_value(*args: str) -> tuple[int, str]:
    try:
...
def _check_tenant_systemd_unit(unit: str) -> CheckResult:
    active_rc, active = _systemctl_value("is-active", unit)
    enabled_rc, enabled = _systemctl_value("is-enabled", unit)
    detail = (
        f"{unit}: active={active or 'unknown'} (rc={active_rc}), "
        f"enabled={enabled or 'unknown'} (rc={enabled_rc})"
    )
    if active_rc == 0 and active == "active":
        return CheckResult(name=f"tenant_unit_{unit}", level="ok", detail=detail)
    fix_action = "enable --now" if unit.endswith(".timer") else "restart"
    return CheckResult(
        name=f"tenant_unit_{unit}",
        level="error",
        detail=detail,
        fix=f"sudo systemctl {fix_action} {unit}",
    )


def _check_tenant_systemd(tenant: str) -> list[CheckResult]:
    return [
        _check_tenant_systemd_unit(f"marvis@{tenant}.service"),
        _check_tenant_systemd_unit(f"marvis-brain@{tenant}.timer"),
        _check_tenant_systemd_unit(f"marvis-kg-watcher@{tenant}.service"),
    ]
...
def _check_connectivity(*, offline: bool) -> CheckResult:
    """TCP connectivity probe to a public package index endpoint.
...
def _check_mcp_double_registration() -> CheckResult:
    """Warn when the Claude Code plugin and local .mcp.json both register Marvis."""
    try:
        from core.cli import marvis_mcp

        plugin = marvis_mcp._detect_claude_plugin()
        keys = marvis_mcp._registered_keys(Path.cwd() / ".mcp.json")
    except Exception as exc:  # noqa: BLE001 — doctor must stay best-effort
        return CheckResult(
            name="mcp_double_registration",
            level="warning",
            detail=f"Could not inspect MCP registration overlap: {exc}",
        )

    if plugin["detected"] and keys:
        return CheckResult(
            name="mcp_double_registration",
            level="warning",
            detail=(
                "Claude Code plugin route detected and local .mcp.json also "
                f"registers {', '.join(keys)}. Claude Code may start the same "
                "Marvis MCP server more than once."
            ),
            fix=(
                "Use one route: keep the Claude Code plugin and remove Marvis "
                "entries from .mcp.json, or re-run `marvis mcp register --force` "
                "only when you are not using the plugin."
            ),
        )
    if plugin["detected"]:
        return CheckResult(
            name="mcp_double_registration",
            level="ok",
            detail="Claude Code plugin detected; no local .mcp.json Marvis entry found",
        )
    if keys:
        return CheckResult(
            name="mcp_double_registration",
            level="ok",
            detail=f"local .mcp.json registers {', '.join(keys)}; plugin not detected",
        )
    return CheckResult(
        name="mcp_double_registration",
        level="ok",
        detail="no overlapping Marvis MCP registration detected",
    )
...
def _check_graph_freshness() -> list[CheckResult]:
    """KG coverage: flag projects whose graph nodes carry no git provenance (#13).

    A project indexed without commit SHAs makes graph_impact / graph_neighbors
    silently under-report the blast radius. Cheap DB-only probe; per-project HEAD
    drift is surfaced live on each graph response, not here.
    """
    import sqlite3
    from pathlib import Path
    ...
    if not db_path.exists():
        return [CheckResult(name, "ok", "No brain DB yet — nothing to check.")]
    ...
    try:
        ...
        try:
            rows = conn.execute(
                ...
                "FROM graph_nodes WHERE project_id IS NOT NULL GROUP BY project_id"
            ).fetchall()
        finally:
            conn.close()
    except sqlite3.Error:
        return [CheckResult(name, "ok", "Knowledge graph not initialised yet.")]

    no_provenance = sorted(r[0] for r in rows if r[1] > 0 and r[2] == 0)
    if not no_provenance:
        return [
            CheckResult(name, "ok", f"{len(rows)} project(s) indexed with commit provenance.")
        ]

    listed = ", ".join(no_provenance[:8])
    return [
        CheckResult(
            name,
            "warning",
            f"{len(no_provenance)} project(s) have graph nodes without git provenance "
            f"({listed}) — graph_impact/neighbors may under-report the blast radius.",
            fix="marvis project index <slug>",
        )
    ]
...
def doctor_cmd(
    ...
    tenant = _tenant_id_from_env()
    if tenant:
        checks.append(
            CheckResult(
                name="tenant_context",
                level="ok",
                detail=f"tenant context detected: {tenant}",
            )
        )
        checks.append(_check_tenant_resolved_paths(tenant))
        checks.extend(_check_tenant_systemd(tenant))
    else:
    ...
    # Onboarding completion (P4): orthogonal to health — "is Marvis fully set up?".
    # Single source in core/cli/_onboarding.py; never changes the exit code.
    from core.cli._onboarding import classify, completion_json

    completion = classify(checks)

    if json_out:
        # Contract: a JSON ARRAY of check objects — the pre-P4 shape every
        # consumer iterates (`doctor --json | jq '.[] | select(.name==...)'`).
        # The P4 onboarding-completion block is ADDITIVE for real: it rides as
        # one extra array element shaped like any check (name/level/detail,
        # full payload under "summary") instead of wrapping the array in an
        # object — the wrapper broke every iterating consumer (caught by the
        # E2E Windows resolved_paths probe, red since 2026-06-09).
        completion_payload = completion_json(completion)
        payload = [c.to_dict() for c in checks]
        payload.append(
            {
                "name": "onboarding_completion",
                "level": "ok" if completion_payload["complete"] else "warning",
                "detail": (
                    "required "
                    f"{completion_payload['required_done']}/{completion_payload['required_total']}"
                    ", recommended "
                    f"{completion_payload['recommended_done']}/{completion_payload['recommended_total']}"
                ),
...
def _render_completion(summary: dict) -> None:
    """Render the onboarding completion section: which binary states are done, the
    required/recommended tallies, and the fix for each open one. Orthogonal to the
    health checks — this never changes the exit code (completeness != error)."""
    from rich.table import Table  # lazy — only for human output
    ...
    for state, done, detail in summary["rows"]:
        mark = "[green]✓ done[/green]" if done else "[yellow]· open[/yellow]"
        table.add_row(state.title, state.tier, mark, detail)

    console.print()
    console.print(table)

    req = f"required {summary['required_done']}/{summary['required_total']}"
    rec = f"recommended {summary['recommended_done']}/{summary['recommended_total']}"
    console.print()
    if summary["complete"]:
        console.print(f"[green bold]✓ Installation complete[/green bold] — {req}.  [dim]{rec}[/dim]")
    else:
        console.print(f"[yellow bold]Installation incomplete[/yellow bold] — {req}.  [dim]{rec}[/dim]")

    open_states = [s for (s, done, _) in summary["rows"] if not done and s.fix]
    if open_states:
        console.print()
        console.print("[bold]To finish setup:[/bold]")
        for state in sorted(open_states, key=lambda s: 0 if s.tier == "required" else 1):
            tag = "required" if state.tier == "required" else "optional"
            console.print(f"  [bold]{state.title}[/bold] ({tag}): {state.fix}")
