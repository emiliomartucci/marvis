...
def project_import_cmd(
    ...
    resolved_slug = slug or src.name
...
def project_index_cmd(
    ...
    no_embed: bool = typer.Option(
        False,
        "--no-embed",
        help="Index the call/import/defines graph only; skip code embeddings "
        "(avoids loading the local embedding model).",
    ),
    json_out: bool = typer.Option(False, "--json", help="Emit pure JSON to stdout."),
) -> None:
    ...
    symlink/size/count), populates calls/imports/defines + a chunk-per-symbol code
    embedding, and proves the source tree is byte-for-byte untouched (D3).
    """
