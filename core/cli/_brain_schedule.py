...
_SCHTASKS_TN = "MarvisX Brain"  # Windows Task Scheduler task name
...
def detect_backend() -> str:
    """Return ``launchd`` | ``systemd`` | ``cron`` | ``schtasks`` | ``unsupported``."""
...
def _marvis_path() -> str | None:
    ...
    if argv0.endswith(("marvis", "marvis.exe", "marvis-init", "marvis-init.exe")) and os.path.isfile(argv0):
...
def _launchd_disable() -> dict[str, Any]:
    plist = _launchd_plist_path()
    rc, _out, err = _run(["launchctl", "bootout", f"gui/{current_uid()}/{_LABEL}"])
    removed = False
    if plist.is_file():
        plist.unlink()
        removed = True
...
def _launchd_status() -> dict[str, Any]:
    plist = _launchd_plist_path()
    rc, _out, _err = _run(["launchctl", "print", f"gui/{current_uid()}/{_LABEL}"])
    return {
        "backend": "launchd",
        "enabled": rc == 0,
...
# ---------------------------------------------------------------------------
# schtasks backend (Windows Task Scheduler)
# ---------------------------------------------------------------------------


def schtasks_argv_create(
    marvis_path: str, *, hour: int = _HOUR, minute: int = _MINUTE, tn: str = _SCHTASKS_TN
) -> list[str]:
    """Build the ``schtasks /create`` argv (PURE — unit-tested).

    ``/tr`` is ONE argv element; the program path keeps its OWN inner quotes so a
    space in ``%USERPROFILE%`` (e.g. ``C:\\Users\\Marsida Curaj\\...``) survives Task
    Scheduler parsing. ``_run`` passes a list (no shell), so subprocess does the
    OUTER quoting via ``list2cmdline``; this builder owns only the inner ``/tr``.
    ``/f`` force-overwrites = idempotent; no ``/ru`` = current user, no admin.
    """
    tr = f'"{marvis_path}" brain run --mode full'
    return [
        "schtasks", "/create", "/tn", tn, "/tr", tr,
        "/sc", "DAILY", "/st", f"{hour:02d}:{minute:02d}", "/f",
    ]


def _schtasks_enable() -> dict[str, Any]:
    marvis = _marvis_path()
    if not marvis:
        return {"backend": "schtasks", "ok": False, "error": "marvis not found on PATH"}
    rc, _out, err = _run(schtasks_argv_create(marvis))
    # schtasks can refuse on a password-protected / Microsoft-account user without
    # stored creds; never crash — the opportunistic on-invocation upkeep is the net.
    return {
        "backend": "schtasks",
        "ok": rc == 0,
        "task_name": _SCHTASKS_TN,
        "error": err.strip() if rc != 0 else None,
        "warning": (
            None
            if rc == 0
            else "could not register the Windows scheduled task; the opportunistic "
            "on-invocation upkeep still runs whenever you use marvis"
        ),
    }


def _schtasks_disable() -> dict[str, Any]:
    # rc!=0 when the task does not exist — still a successful disable.
    rc, _out, _err = _run(["schtasks", "/delete", "/tn", _SCHTASKS_TN, "/f"])
    return {"backend": "schtasks", "ok": True, "deleted_rc": rc}


def _schtasks_status() -> dict[str, Any]:
    rc, _out, _err = _run(["schtasks", "/query", "/tn", _SCHTASKS_TN])
    return {"backend": "schtasks", "enabled": rc == 0}
...
_ENABLE = {"launchd": _launchd_enable, "systemd": _systemd_enable, "cron": _cron_enable, "schtasks": _schtasks_enable}
_DISABLE = {"launchd": _launchd_disable, "systemd": _systemd_disable, "cron": _cron_disable, "schtasks": _schtasks_disable}
_STATUS = {"launchd": _launchd_status, "systemd": _systemd_status, "cron": _cron_status, "schtasks": _schtasks_status}
