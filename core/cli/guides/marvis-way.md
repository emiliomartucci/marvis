...
- **CLI installed and on PATH** — `marvis` resolves on your shell.
- **Config present and valid** — `marvis init` created a readable `settings.yaml`.
- **MCP server registered in Claude Code** — your agent can call the tools (`marvis mcp`).

**Recommended** (each unlocks more):

- **Governance hooks installed** — the safety/quality gates in your repo (`marvis hooks install`).
- **At least one project imported** — Marvis has something to work on (`marvis project import <path>`).
- **Code indexed (projects with code)** — the Knowledge Graph is populated (`marvis project index <slug>`).
- **Brain enabled and scheduled** — reflection runs on a schedule (`marvis brain enable`).
- **LLM configured (brain not mute)** — the brain has a model to write with (an API key, the local model, or the `claude -p` path).
