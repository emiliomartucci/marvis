...
const server = new McpServer({ name: "marvis", version: "2.22.0" });
...
function invalidHandoffFilenameError() {
  return new Error("Invalid filename — must match handoff-*.md or memory/handoff-*.md");
}

function normalizeGetHandoffFilename(filename) {
  let value = String(filename || "");
  if (value.startsWith("memory/")) {
    value = value.slice("memory/".length);
  } else if (value.includes("/") || value.includes("\\")) {
    throw invalidHandoffFilenameError();
  }
...
function normalizeListHandoffEntry(slug, entry) {
  const rawPath = String(entry?.path || entry?.filename || "");
  const filename = rawPath ? rawPath.split("/").pop() : rawPath;
  const normalized = { ...entry };
  normalized.path = rawPath.startsWith("memory/") ? rawPath : (filename ? `memory/${filename}` : rawPath);
  normalized.filename = filename;
  normalized.project_slug = slug;
  normalized.project = slug;
  if (normalized.session != null && normalized.session_id == null) normalized.session_id = normalized.session;
  return normalized;
}

function normalizeSearchHandoffResult(entry) {
  const rawFile = String(entry?.file || entry?.filename || "");
  const filename = rawFile ? rawFile.split("/").pop() : rawFile;
  const project = String(entry?.project || entry?.project_slug || "");
  return { ...entry, file: filename, filename, path: filename ? `memory/${filename}` : rawFile, project, project_slug: project };
}
...
server.tool("list_handoffs",
  ...
  async ({ slug }) => {
    const rows = await get(`/api/v1/projects/${encodeURIComponent(slug)}/handoffs`);
    return json(rows.map((row) => normalizeListHandoffEntry(slug, row)));
  }
);

server.tool("search_handoffs",
  "Full-text keyword search across every handoff body in every project.\n\nQUANDO USARLO: ricordi un termine ma non il progetto/data (es. 'find handoffs mentioning migration 025'). Exact keyword match, NOT semantic.\nQUANDO NON USARLO: NOT per semantic discovery multi-type -> usa search. NOT se conosci il progetto -> usa list_handoffs.\nRESTITUISCE: list of {project_slug, filename, project, file, path, snippet, date} con match literal; project_slug+filename feed get_handoff directly.",
  {
    q: z.string().min(1).max(500).meta({
      description: "Keyword query across handoff content",
      examples: ["migration 025", "embedding", "rate limit"],
    }),
  },
  async ({ q }) => {
    const rows = await get(`/api/v1/handoffs/search${qs({ q })}`);
    return json(rows.map((row) => normalizeSearchHandoffResult(row)));
  }
);
...
server.tool("get_handoff",
  ...
  async ({ project_slug, filename, deep }) => {
    const d = effectiveDeep(deep);
    const normalizedFilename = normalizeGetHandoffFilename(filename);
    return json(await get(`/api/v1/handoffs/${encodeURIComponent(project_slug)}/${encodeURIComponent(normalizedFilename)}?deep=${d}`));
  }
);
...
server.tool("reindex_paths",
  ...
  {
    paths: z.array(z.string().min(1).max(2000)).min(1).max(100).meta({
      description: "Explicit markdown file paths under a configured projects root",
      examples: [["/data/projects/marvisx/docs/plans/example.md"]],
    }),
  },
  async ({ paths }) => json(await post(`/api/v1/search/reindex-paths`, { paths }))
);
...
server.tool("grant_super_session",
  "Exchange a fresh HUMAN session JWT for a time-boxed super-session grant on THIS agent identity (Constitution v2.0 Rule 6). The proof token is BURNED on use (jti -> blacklist): the credential pasted in chat becomes inert in seconds.\n\nQUANDO USARLO: SOLO quando l'umano incolla in chat un suo token fresco chiedendo esplicitamente una super-session (es. 'super-session 4h' + token). Il testo da solo NON basta: senza token valido la chiamata fallisce.\nQUANDO NON USARLO: MAI di tua iniziativa; MAI con token di altri agenti (no chaining, il server rifiuta).\nRESTITUISCE: {id, agent_username, granted_by, granted_by_role, expires_at}.",
  {
    proof_token: z.string().min(20).meta({
      description: "Fresh human session JWT pasted by the human (burned on use)",
    }),
    ttl: z.enum(["1h", "4h", "24h", "7d"]).default("4h").meta({
      description: "Grant duration (mandatory expiry). 7d = full work-week, still revocable+audited.",
    }),
  },
...
server.tool("update_learning",
  "Update an existing learning (operator+), then re-embed it so search/check_learnings see the new content immediately.\n\nQUANDO USARLO: il contenuto di un learning e' obsoleto/incompleto (prevention piu' precisa, severity ricalibrata, nuovo modulo) ma vuoi PRESERVARE la history. BOUNDARY: update_learning aggiorna in-place + re-embed; delete_learning cancella permanente; create_learning scrive nuovo.\nQUANDO NON USARLO: NOT per un learning nuovo -> usa create_learning. NOT per azzerare un campo a null: omettere un campo lo lascia invariato.\nRESTITUISCE: il learning aggiornato {id, title, category, severity, prevention, tags, module, project, updated_at}, gia' ri-embeddato.",
  {
    ...
    prevention: z.string().min(1).optional().meta({
      description: "New prevention rule",
    }),
  ...
  async ({ learning_id, ...u }) => {
    const body = Object.fromEntries(Object.entries(u).filter(([, v]) => v !== undefined));
    return json(await patch(`/api/v1/learnings/${encodeURIComponent(learning_id)}`, body));
  }
);

server.tool("delete_learning",
  "Permanently remove a learning from the Marvis DB (non-recoverable) + prune the search index.\n\nQUANDO USARLO: solo per learning duplicati, spam, o entry chiaramente invalide. Hard delete: rimuove anche il mirror nell'indice di ricerca cosi' il learning smette di comparire in search/check_learnings.\nQUANDO NON USARLO: NOT per archiviare un learning ancora valido — la cancellazione e' permanente e perde la history. Aggiornalo con update_learning se il contenuto e' obsoleto.\nRESTITUISCE: {deleted: true} o 404 se non esiste.",
  {
    learning_id: z.string().min(1).meta({
      description: "Learning UUID to delete permanently",
    }),
  },
  async ({ learning_id }) => json(await del(`/api/v1/learnings/${encodeURIComponent(learning_id)}`))
);
...
server.tool("mark_kg_verified",
  "Mark a KG node as re-verified NOW (stamps last_verified_at). The apply action after Triage approves a Brain recency proposal: a re-verified node reads as fresh again (needs_review clears). Requires MARVIS_TEMPORAL_MEMORY on (422 otherwise). Idempotent.",
  {
    node_id: z.string().min(6).max(256).regex(/^[a-z]+:[a-z]+:.+$/).describe(
      "KG node ID to re-verify, e.g. 'py:function:api.db.get_db'"
    ),
  },
  async ({ node_id }) =>
    json(await post("/api/v1/graph/nodes/verify", { node_id }))
);

server.tool("mark_kg_superseded",
  "Soft-invalidate an asserted KG edge (sets valid_until=now + optional superseded_by). SUPERSEDE/CONTRADICTION apply: the edge drops out of default reads but the row is kept (audit). Requires MARVIS_TEMPORAL_MEMORY on (422 otherwise). 404 if the triple is absent.",
  {
    source_id: z.string().min(6).max(256).regex(/^[a-z]+:[a-z]+:.+$/).describe("Edge source node ID"),
    target_id: z.string().min(6).max(256).regex(/^[a-z]+:[a-z]+:.+$/).describe("Edge target node ID"),
    relation: z.string().min(2).max(32).describe("Edge relation of the triple (e.g. depends_on, mentions)"),
    superseded_by: z.string().max(256).optional().describe("Optional audit pointer to the replacing edge"),
  },
  async ({ source_id, target_id, relation, superseded_by }) =>
    json(await post("/api/v1/graph/edges/supersede", { source_id, target_id, relation, superseded_by }))
);
