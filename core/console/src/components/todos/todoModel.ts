...
export type TodoFilter = "all" | "decisions" | "promemoria" | "idea" | "brain" | "completati";
...
export interface TodoActionDefinition {
  ...
  /**
   * Decidi gate: confirming opens a decision form (choice + rationale +
   * options) that is persisted into the todo payload, so the backend writes a
   * non-empty ADR (gh #29). Without it the ADR would record an empty decision.
   */
...
export function matchesTodoFilter(todo: TodoResponseLocal, filter: TodoFilter): boolean {
  ...
  if (filter === "completati") return todo.status === "fatto";
...
/**
 * Completed todos (status=fatto), most-recently-finished first (gh #34).
 * Completion has no due-date semantics, so this list is flat (not horizon-
 * grouped) and ordered by ``updated_at`` — the closest proxy for "done at".
 */
export function completedTodos(todos: TodoResponseLocal[]): TodoResponseLocal[] {
  return todos
    .filter((todo) => todo.status === "fatto")
...
/**
 * True when this todo was typed/projected by the heuristic fallback rather than
 * the advanced LLM classifier (gh #22). Drives a discreet "basic classification"
 * footnote so the user knows the auto-classification is degraded, not wrong.
 */
export function isHeuristicClassified(todo: Pick<TodoResponseLocal, "payload">): boolean {
  return todo.payload?.classified_by === "heuristic";
}
...
export function todoActionDefinitions(todo: TodoResponseLocal): TodoActionDefinition[] {
  ...
  if (todo.type === "decidi") {
    return [
      { id: "confirm", tone: "primary", needsDecision: true },
