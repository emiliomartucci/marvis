...
describe("todoModel", () => {
  ...
  it("collects completed todos newest-first and filters by completati (gh #34)", () => {
    const items = [
      todo({ id: "open-1", status: "aperto", updated_at: "2026-06-12T09:00:00Z" }),
      todo({ id: "done-old", status: "fatto", updated_at: "2026-06-10T09:00:00Z" }),
      todo({ id: "done-new", status: "fatto", updated_at: "2026-06-12T09:00:00Z" }),
    ];
    expect(completedTodos(items).map((item) => item.id)).toEqual(["done-new", "done-old"]);
    expect(matchesTodoFilter(todo({ status: "fatto" }), "completati")).toBe(true);
    expect(matchesTodoFilter(todo({ status: "aperto" }), "completati")).toBe(false);
  });

  it("marks the decidi confirm action as needing a decision form (gh #29)", () => {
    const actions = todoActionDefinitions(todo({ type: "decidi" }));
    const confirm = actions.find((action) => action.id === "confirm");
    expect(confirm?.needsDecision).toBe(true);
    // Other gates must NOT trigger the decision form.
    const azioneComplete = todoActionDefinitions(todo({ type: "azione" })).find(
      (action) => action.id === "complete"
    );
    expect(azioneComplete?.needsDecision).toBeUndefined();
  });
  ...
  it("flags only heuristic-classified todos", () => {
    expect(isHeuristicClassified(todo({ payload: { classified_by: "heuristic" } }))).toBe(true);
    expect(isHeuristicClassified(todo({ payload: { classified_by: "llm" } }))).toBe(false);
    expect(isHeuristicClassified(todo({ payload: null }))).toBe(false);
  });
