  ...
  return (
    ...
    >
      <div className="px-4 pb-3 pt-4">
        <Link
          href="/brain/diario/"
          className="flex items-center gap-2 text-pir-text-primary transition-opacity hover:opacity-80"
        ...
        >
          ...
          {/* eslint-disable-next-line @next/next/no-img-element -- Local shell brief requires reusing public SVG assets directly. */}
          <img
            src={`${APP_BASE_PATH}/logo-lockup.svg`}
...
export default function AppShell({ children, sidebar }: AppShellProps) {
  if (isLocalMode()) {
    return <LocalAppShellContent sidebar={sidebar}>{children}</LocalAppShellContent>;
  }
