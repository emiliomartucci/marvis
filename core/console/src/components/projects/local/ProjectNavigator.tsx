...
function ProjectColorDot({ color }: { color: string | null | undefined }) {
  return (
    <span
      aria-hidden
      className={`h-2 w-2 shrink-0 rounded-full ${color ? "" : "bg-pir-border-strong"}`}
  ...
  useEffect(() => {
    ...
    getPrograms({ signal: controller.signal })
      .then((nextPrograms) => {
        ...
        setCollapsed((current) => current ?? new Set(nextPrograms.map((program) => program.name)));
      })
      .catch(() => {})
      .finally(() => {
    ...
    return programs
      .map((program) => ({
        ...
        projects: program.projects.filter((project) =>
          project.on_server && projectMatches(project, normalizedQuery)
        ),
      }))
      .filter((program) => program.projects.length > 0);
  }, [normalizedQuery, programs]);

  const visibleCount = visiblePrograms.reduce((sum, program) => sum + program.projects.length, 0);
    ...
    <section className="flex h-full min-h-0 flex-col" aria-label={t.appShell.projectsSlotLabel}>
      ...
      <div className="mt-3">
        <input
          ...
          className="h-8 w-full rounded border border-pir bg-pir-base px-2 text-caption text-pir-text-primary outline-none transition-colors placeholder:text-pir-text-muted focus:border-pir-accent"
        />
      </div>
            ...
            return (
              <div key={program.name}>
                ...
                >
                  <span
                    aria-hidden
                    className={`text-pir-text-muted transition-transform ${isCollapsed ? "" : "rotate-90"}`}
                  >
                    &gt;
                  </span>
                    ...
                    {program.projects.map((project) => {
                      ...
                      return (
                        <Link
                          ...
                          className={`flex h-8 items-center gap-2 rounded px-2 text-caption transition-colors ${
                            active
