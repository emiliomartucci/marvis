...
// Deterministic dd/mm/yyyy hh:mm — toLocaleString output depends on the
// runtime's ICU build, so the same ISO string can hydrate differently than it
// prerendered (React #418 on the static export). Manual formatting is
// environment-independent for a given timezone.
      ...
      <header className="flex flex-wrap items-baseline justify-between gap-2 font-[var(--font-jetbrains-mono)] text-[11px] uppercase tracking-[0.18em] text-pir-text-tertiary">
        ...
        {view.published_at && (
          <span className="normal-case tracking-normal text-pir-text-tertiary">
            pubblicato {formatPublishedAt(view.published_at)}
          </span>
        )}
      </header>
