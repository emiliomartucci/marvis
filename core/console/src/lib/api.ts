...
export async function getProjectDetail(slug: string, opts?: { signal?: AbortSignal }): Promise<ProjectDetail> {
  return fetchAPI<ProjectDetail>(`/api/v1/projects/${encodeURIComponent(slug)}`, { signal: opts?.signal });
}
...
export async function listTasks(
  ...
  return fetchAPI<TaskResponse[]>(`/api/v1/tasks${qs ? `?${qs}` : ""}`, {
    signal: opts?.signal,
  });
}
...
export async function getCostsSummary(
  ...
  const qs = searchParams.toString();
  return fetchAPI<ProjectCostSummary[]>(`/api/v1/costs/summary${qs ? `?${qs}` : ""}`, {
    signal: opts?.signal,
  });
}
...
export async function getProjectCosts(
  ...
  return fetchAPI<ConversationCost[]>(
    `/api/v1/costs/by-project/${encodeURIComponent(slug)}${qs ? `?${qs}` : ""}`,
    { signal: opts?.signal }
  );
}
...
export async function getProjectBilling(
  ...
  return fetchAPI<ProjectBillingSummary>(
    `/api/v1/costs/billing/${encodeURIComponent(slug)}${qs ? `?${qs}` : ""}`,
    { signal: opts?.signal }
  );
}
...
export async function listUsers(params?: { project?: string }): Promise<User[]> {
  ...
  const qs = searchParams.toString();
  return fetchAPI<User[]>(`/api/v1/users${qs ? `?${qs}` : ""}`);
}
...
export async function listExecutions(
  ...
  if (params?.limit) searchParams.set("limit", String(params.limit));
  const qs = searchParams.toString();
  return fetchAPI(`/api/v1/automations/executions${qs ? `?${qs}` : ""}`, {
    signal: opts?.signal,
  });
}
...
export async function listOutboxEvents(
  ...
  if (params?.limit) searchParams.set("limit", String(params.limit));
  const qs = searchParams.toString();
  return fetchAPI(`/api/v1/automations/events${qs ? `?${qs}` : ""}`, {
    signal: opts?.signal,
  });
}
...
export async function listNotifications(
  ...
  return fetchAPI<Notification[]>(`/api/v1/notifications${qs ? `?${qs}` : ""}`, {
    signal: opts?.signal,
  });
}
...
export async function getCodexModules(
  ...
  const qs = params.toString();
  return fetchAPIValidated(
    `/api/v1/graph/codex-modules${qs ? `?${qs}` : ""}`,
    CodexModulesResponseZ,
    { signal: opts.signal }
  );
}
