...
export class ReconnectingTerminalWS {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      const probePromise = this.probeDirectPath().then((result) => {
        ...
        this.emitLifecycle({
          ...
          transport: result === directWsUrl ? "direct" : "tunnel",
        });
        return result;
      });
      ...
      this.emitLifecycle({
        ...
        transport: wsBase === getDirectWsBaseUrl() ? "direct" : "tunnel",
      });
    } catch (err) {
    ...
    const transport = wsBase === getDirectWsBaseUrl() ? "direct" : "tunnel";
    ...
    ws.onopen = () => {
      socketOpened = true;
      this.isDirect = wsBase === getDirectWsBaseUrl();
    ...
    ws.onclose = (event) => {
      ...
      if (wsBase === getDirectWsBaseUrl() && !this.isDirect) {
  ...
  private async probeDirectPath(): Promise<string> {
    ...
    try {
      const probeUrl = getDirectWsProbeUrl();
      if (!probeUrl) return wsBaseUrl;
      const resp = await fetch(probeUrl, {
        mode: "cors",
        cache: "no-store",
        signal: this.probeController.signal,
      });
      return resp.ok ? directWsUrl : wsBaseUrl;
    } catch {
      return wsBaseUrl;
    } finally {
