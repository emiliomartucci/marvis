...
def _enabled() -> bool:
    """Opt-in gate — runs before any network/queue/disk init (mem0 lesson).
    ...
    5. else → ``settings.yaml telemetry:`` (default OFF if unset).
    """
    ...
    return _settings_telemetry_on()  # default OFF (opt-in)
...
def _settings_telemetry_on() -> bool:
    """Default-OFF: telemetry is opt-in — it stays off unless ``settings.yaml``
    explicitly turns it on.

    Accepts both shapes the codebase uses for the flag:
    - ``telemetry: true`` (a bare bool), or
    - ``telemetry: {enabled: true}`` (a nested dict).
    Anything else (or no key) → OFF.
    """
    data = _read_settings()
    tele = data.get("telemetry")
    if isinstance(tele, dict):
        return bool(tele.get("enabled", False))
    if isinstance(tele, bool):
        return tele
    return False  # default off (opt-in): key absent / unexpected type
...
def maybe_first_run_notice() -> None:
    """Print the one-time telemetry notice to stderr (then never again).

    Telemetry is OPT-IN (off by default). On the first run we tell the user it is
    off and how to turn it on; if they have already opted in, we confirm it is on
    and how to turn it back off. Either way the flag is written so it never
    repeats. NOT a blocking prompt. The flag lives in
    ``~/.marvis/telemetry_notice_shown``. Writing the flag is the only side
    effect — no install id is coined and nothing is sent here.
    """
    ...
    if not _log_mode():
        ...
        if _enabled():
            sys.stderr.write(
                "Anonymous, opt-in telemetry is on (no content, no PII). "
                "Turn it off with `marvis telemetry off`.\n"
            )
        else:
            sys.stderr.write(
                "Telemetry is off by default. To help improve Marvis with "
                "anonymous usage counts (no content, no PII), turn it on with "
                "`marvis telemetry on`.\n"
            )
