...
EVENT_PROPS: dict[str, frozenset[str]] = {
    ...
    # Whether the Marvis MCP server got registered (a bool, not the path/entry).
    "mcp_registered": frozenset({"registered"}),
    # Claude Code plugin install marker. Emitted by the /marvis:install skill only
    # when telemetry is enabled; no props, so it cannot carry install context.
    "plugin_installed": frozenset(),
}
